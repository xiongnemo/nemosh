...
func newRuntimeWithState(registry applets.Registry, streams Streams, state State, paths pathState, initErr error) Runtime {
	...
	created := Runtime{initErr: initErr, registry: registry, functions: map[functionName]functionDefinition{}, streams: fds.streams(), fds: fds, vars: variables, traps: map[trapName]string{}, trapRunning: map[trapName]bool{}, params: &parameters{}, options: &shellOptions{}, expansion: &expansionState{}, aliases: map[string]string{}, readonly: map[string]struct{}{}, mask: newFileModeMask(), paths: &paths, env: state.Env.clone(), jobScope: newRootJobScope(), lifecycle: &shellLifecycle{}}

...
var usageText = map[string]Usage{
	...
	"nano":       {Summary: "Edit a file, with nano's key bindings.", Operands: "[FILE]", Options: map[string]string{"H": "list the features this build implements", "R": "open read-only", "l": "show line numbers; they are already on"}, Notes: []string{"One editor under two names: nano binds ^O ^X ^W ^K ^U ^G and ^\\ to replace, micro binds ^S ^Q ^F ^K ^V ^G and ^R. A subset, and -H lists exactly which -- in the manner of `busybox vi -H`, whose own vi is a 3000-line clone that says so."}},
	"micro":      {Summary: "Edit a file, with micro's key bindings.", Operands: "[FILE]", Options: map[string]string{"H": "list the features this build implements", "R": "open read-only", "l": "show line numbers; they are already on"}, Notes: []string{"The same editor as nano with a Windows-shaped key map: ^S saves, ^Q quits, ^F finds, ^R replaces. -H lists what is implemented and what is not."}},

...
func TestRunInteractive_interruptsCatBeforeFreshInputAndPreservesFreshLine(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "plain stdin", command: "cat"},
		{name: "device stdin", command: "cat /dev/stdin"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := io.WriteString(writer, commandLine); err != nil {
				t.Fatalf("write cat command: %v", err)
			}
			awaitOutstandingRead(t, trackedInput, base+uint64(len(commandLine)))
			...
			if !awaitTextCountBounded(stderr, "P> ", 2, time.Second) {
				closeInput()
				awaitError(t, done, "interrupted cat cleanup")
			...
			closeInput()
			if err := awaitError(t, done, "interactive cat recovery"); err != nil {
				t.Fatalf("interactive run: %v", err)
			}
			if got := stdout.String(); got != "fresh\n" {
				t.Fatalf("stdout = %q, want %q", got, "fresh\n")
			}
			if strings.Contains(stderr.String(), "cat:") {
				t.Fatalf("stderr = %q, want no cat cancellation diagnostic", stderr.String())
			}
		})
	}
}

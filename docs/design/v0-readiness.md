...
| Wave B | `/dev/clipboard`, fixed suffix lookup, `ComSpec` batch launch, `.sh`/shebang/CRLF dispatch, applet override configuration. | **Partial.** Suffix lookup, batch launch through `ComSpec`, `.sh`/shebang dispatch, CRLF handling down to a preserved lone `\r`, and `/dev/clipboard` are complete; see the Windows launch rows and the device-set row above. Applet override configuration remains. |

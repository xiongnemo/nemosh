...
| Control flow | **Complete for the selected v0 grammar** | `internal/shell/runtime/execute_ast.go`, `execute_compound.go`, `execute_group.go`, `function.go`, `parser_case_lines.go`, `parser_elif.go`, `pattern.go`; `control_test.go`, `case_test.go`, `case_one_line_test.go`, `elif_test.go`, `sequential_list_test.go`, `pipeline_negation_test.go`, `assignment_prefix_test.go` | Brace groups and function calls propagate control; functions consume return at the nearest function boundary, sourced files at their own, and subshells isolate escaping control. Case arms match by POSIX 2.13.1 pattern with `\|` alternatives, not by string equality with a special case for a lone `*`. A leading assignment no longer hides `break`, `continue`, `exit`, `return` or `exec` from dispatch -- `while true; do V=x break; done` used to run forever. | `break n` and `continue n` honour their count now; the operand used to be ignored. What remains is in the known-gaps table at the end: a compound command as the condition of `if` or `while`, and piping into a `case`. |
| Special/stateful builtins | **Partial** | Implementations under `internal/shell/runtime/`; tests include `special_builtin_test.go`, `exec_test.go`, `exec_redirect_test.go`, `set_builtin_test.go`, `errexit_nounset_test.go`, `trap_builtin_test.go`, `cd_builtin_test.go`, `command_v_test.go`, `return_test.go`, and the P0.4 job tests. | `set` parses the POSIX options and refuses an unknown one; `-e` and `-u` act, and `$-` and `set -o` report the rest. `trap` lists, arms and resets. `cd` honours HOME, `-`, PWD and OLDPWD. A redirect-only `exec` rebinds the shell. `command -v` reaches PATH. `:` exists. `alias`, `unalias`, `type` and `local` landed on 2026-08-07. `times` and `let` have since landed. **Absent, and loudly so:** `hash`, `ulimit`, `fg` and `bg` say `not implemented` with the reason, and status 126. `fg` and `bg` match busybox-w32, which compiles them out on Windows (`#if JOBS`, and `JOBS` is 0 under `ENABLE_PLATFORM_MINGW32`, shell/ash.c:246-252). `ulimit` deliberately does *not* match it: busybox-w32 keeps the name and stubs `shell_builtin_ulimit` to `return 1` (shell/shell_common.c), so `ulimit -n` there fails with no message and no reason — the silent-stub pattern this ledger's rule exists to forbid. `-a`, `-C`, `-x`, `-f`, `-e` and `-u` act. `-n` and `-v` are refused by name, because a script is parsed in full before any of it runs, so there is no unread input left for either to act on. **Stored but inert:** `-b` is remembered and reported by `$-` and `set -o`, and nothing reads it. |
...
| Traps/signals | **Complete for P0.4** | `internal/shell/runtime/interrupt.go`, `trap_builtin.go`, `builtins.go`, `script.go`, `interactive.go`; `cmd/nemosh/signal.go`, `main.go`, `session.go`; `trap_builtin_test.go` plus focused interrupt, direct-applet, external-helper, and Windows acceptance tests. | Exactly shell-level EXIT and INT, and since then ERR and RETURN, which are not signals. RETURN runs as a function returns or a sourced file finishes, with bash's rule that a function does not inherit it unless `set -T` (return_trap.go). ERR runs where `set -e` would act, with `set -E` (errtrace) carrying it into functions and subshells, in both references' terms (`trap_err_test.go`). HUP, QUIT and TERM can be trapped since `kill` can send them to a background job, which runs the trap as bash does (`signal_job_test.go`), and a script gets them from outside -- a real `kill` off Windows, the console closing on Windows (`signal_shell_test.go`, `terminations_other_test.go`). A `trap` operand naming a real signal this shell cannot deliver, such as USR1, says so rather than calling the name invalid; one that is not a signal at all keeps bash's `invalid signal specification`. Targeted POSIX child SIGINT, ConPTY, and idle-input handling are not promised. |
...
| --- | --- | --- |
| `>(cmd)`: reading its path blocks, and a script waits for it | do not read a `>(cmd)` path; end a long-running one yourself | `>(cmd)` works, over a named pipe (`//./pipe/nemosh-...`) or a FIFO, with the command reading while the consumer writes (output_substitution.go). Three differences from bash remain. Opening the path to *read* blocks, where bash refuses it: the pipe is duplex so that writers opening it read-write, as Go's `os.Create` and .NET's default FileStream do, are not refused. A script waits for the command before it exits, because here it is part of the shell rather than a process left running. And an unopened path is opened and closed after the command, so the command sees an empty input. `<(cmd)` is still a temporary file; see process_substitution.go |
| `$LINENO` after a line continued inside a list -- `a &&` with `b` on the next line | none | `$LINENO` itself works, and matches busybox in functions, groups, substitutions, past heredocs, in `eval`, traps, a sourced file, `PS4` and a session (line_numbers.go). The one miss is b here: it reports the line its list began on, because joining the lines leaves no trace of where the second began. Both references say b's own line |
| `$!` is a job spec, `%1`, not a process id -- **only under `NEMOSH_JOBS=goroutine`** | `wait $!` and `kill $!` work with it; writing it to a pidfile or handing it to `tasklist` does not | By default a background job is a process, as busybox-w32's are, and `$!` is its pid (background-processes.md). The goroutine it used to be is kept as the way back, and has no pid to report. See support-matrix.md, "Process control" |
...
| `coproc` | a named pipe, or two redirections | **Implemented** (coproc.go), as bash's: an ordinary background job whose group redirects its stdin and stdout to two pipes, the shell's ends on 63 and 60. It runs under either launcher. It was refused by name with 126 |
...
| `trap DEBUG` | none | Refused by name. It runs before every command with `$BASH_COMMAND`, the command as written, and a parsed command here keeps no written form. `trap RETURN`, `trap -p` and `trap -l` work, as in bash |
| `cmd {fd}>file` on a command other than `exec` | `exec {fd}>file` | The descriptor is allocated and the name set, but it lasts only for that command; bash keeps it open until closed |
| `ls -l` shows the same name for owner and group, and a directory's link count is 1 | none needed | busybox does the same with the names, and Windows disagrees with both: the real primary group of a file owned by a local account is `None`, which would read as a fault. The link count is the one thing left: POSIX makes a directory's count 2 plus its subdirectories, and busybox counts them, which costs an enumeration per directory in a listing. The mode string is still Go's `-rw-rw-rw-` where busybox says `-rw-rw-r--` |

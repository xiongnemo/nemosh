...
| `wc` does not pad its count columns | none needed | `wc f` prints `0 1 1 f` where both references align the columns, and they disagree on the width -- busybox pads to 9, GNU and uutils to 7. Cosmetic, and picking one arbitrarily would pin an arbitrary choice |
| `sed` matches a **literal** string, not a regular expression | `grep` for matching; `${x//a/b}` for a literal replacement | `s/.*x/y/` replaces nothing and says nothing, which is the worst shape a gap can take. The substitution is a `strings.Index` search, so every metacharacter is itself. Nothing documents this, and neither the matrix nor `--help` hints at it. Found while writing a fixture that used `sed 's|.*/dir1/|dir1/|'` and got its input back unchanged. Fixing it means translating POSIX BRE into Go's syntax -- `\(`, `\{`, and the GNU `\+`/`\?`/`\|` extensions -- and refusing `\1` backreferences by name, because RE2 has none |

...
| an *unquoted* group in a `[[ =~ ]]` regex | quote it: `[[ x =~ "(b)" ]]`, which works here though bash reads a quoted regex as a literal | The parenthesis is read as a bracket before the condition is parsed; case_awareness.go says where the fix starts. The captures themselves are kept now, in BASH_REMATCH |
